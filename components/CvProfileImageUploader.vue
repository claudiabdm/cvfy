<template>
    <div class="flex">
        <label tabindex="0" for="profile-image-uploader" class="form__btn basis-full">
            {{ isPhotoLoading ? 'Loading...' : $t('upload-profile-image') }}
            <input id="profile-image-uploader" type="file" accept="image/*" name="uploadProfileImage" class="hidden"
                @change="uploadImage" />
        </label>
        <button class="form__btn flex flex-col justify-center" type="button" @click="clearProfileImage">
...
function resizeImageFromReader(readerResult: string) {
    ...
    return new Promise<string>((resolve) => img.addEventListener("load", (e) => {
